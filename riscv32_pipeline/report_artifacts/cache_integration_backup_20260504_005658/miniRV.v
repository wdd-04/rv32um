...
wire            TYPE_M_ID, TYPE_M_ID_INSTDIV, TYPE_M_EX;
wire [2: 0]     M_FUNC3_ID, M_FUNC3_ID_INSTDIV, M_FUNC3_EX;
...
wire [31: 0]    pred_target_IF, pred_target_ID, pred_target_EX;

// 例外信号
wire risk_Ctrl;
wire stop_IF;
wire stop_IF_exception;
wire stop_ID;
wire M_stall_EX;
wire mem_sync_stall;
...
always @(*) begin
    // A操作数�?�择
    if (MUX_A_Forwarding == `ASEL_FORWARDING) begin // 有前�????
...
always @(*) begin
    // A操作数�?�择
    if (MUX_B_Forwarding == `BSEL_FORWARDING) begin // 有前�????
...
EX_MEM ex_mem (
           ...
           .COMPOut(COMPOut_EX),
           .ALUOut(ALUOut_EX),
           .DRAMIn(Bnum2_EX_AfterForwarding), // 对于 s 型指令，必有立即数相加，则用 Bnum2 可以得到寄存器或者前递数�????
           .Unsigned(Unsigned_EX),
...
always @(posedge clk) begin
    bus_rd_WB <= DRAMRd_MEM; // 单独延时处理，降低扇�????
end
...
assign COMPExOut_WB = {31'b0, COMPOut_WB[0]};// 对比较结果进行符号扩�????
...
ForwardingUnit forwardingUnit (
                   .clk(clk),
                   .rst_n(rst_n),
                   .risk_Ctrl(risk_Ctrl), // 确保这一行连上了顶层的跳转信�?
                   .rs1_ID(rs1_EX),
                   .rs2_ID(rs2_EX),
                   .wr_EX(RegWr_MEM),
                   .wr_MEM(RegWr_WB),
