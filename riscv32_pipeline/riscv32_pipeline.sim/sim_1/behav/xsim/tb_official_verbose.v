...
module tb_official_verbose;
    ...
    initial begin
        log_file = $fopen("official_verbose.log", "w");
        for (i = 0; i < 16384; i = i + 1) begin
            dmem[i] = 32'h00000000;
        end

        $readmemh("official_test.mem", tb_official_verbose.u_dut.If.IF_irom.irom.inst.ram_data);
    ...
    always @(posedge clk) begin
        if (rst_n && wb_have_inst) begin
            $fdisplay(log_file,
                "COMMIT t=%0t wb_pc=%h wb_we=%b wb_rd=x%0d wb_val=%h IF_pc=%h IF_inst=%h ID_pc=%h ID_inst=%h EX_pc=%h EX_alu=%h mem_we=%b mem_addr=%h mem_wd=%h mem_rd=%h",
                $time, wb_pc, wb_ena, wb_reg, wb_value,
                u_dut.pc_IF, u_dut.inst_IF, u_dut.pc_ID, u_dut.inst_ID,
                u_dut.pc_EX, u_dut.ALUOut_EX, mem_we, mem_addr, mem_wd, mem_rd);
        end
