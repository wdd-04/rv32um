...
#define STATE_LOG(...) ee_printf(__VA_ARGS__)
#else
#define STATE_LOG(...) do { } while (0)
#endif

static ee_u32 g_state_log_count = 0;
/* local functions */
                 ...
                 ee_u16 crc)
{
    ...
    ee_u8 *p = memblock;
    ee_u32 i;
    ee_u32 log_this = (g_state_log_count < 16);
    if (log_this)
        STATE_LOG("[ST] BENCH_ENTER blksize=%lu mem=0x%08lx seed1=0x%04x seed2=0x%04x step=0x%04x crc=0x%04x\r\n",
    ...
    if (log_this) STATE_LOG("[ST] PASS1_BEGIN p=0x%08lx first=0x%02x\r\n", (long unsigned)(ee_ptr_int)p, *p);
    while (*p != 0)
    {
...
#endif
    if (log_this) STATE_LOG("[ST] PASS1_DONE p=0x%08lx\r\n", (long unsigned)(ee_ptr_int)p);
    p = memblock;
    if (log_this) STATE_LOG("[ST] CORRUPT_BEGIN\r\n");
    ...
    if (log_this) STATE_LOG("[ST] CORRUPT_DONE\r\n");
    p = memblock;
    /* run the state machine over the input again */
    if (log_this) STATE_LOG("[ST] PASS2_BEGIN p=0x%08lx first=0x%02x\r\n", (long unsigned)(ee_ptr_int)p, *p);
    while (*p != 0)
    {
        enum CORE_STATE fstate = core_state_transition(&p, track_counts);
        final_counts[fstate]++;
...
#endif
    if (log_this) STATE_LOG("[ST] PASS2_DONE p=0x%08lx\r\n", (long unsigned)(ee_ptr_int)p);
    p = memblock;
    if (log_this) STATE_LOG("[ST] RESTORE_BEGIN\r\n");
    while (p < (memblock + blksize))
    ...
    if (log_this) STATE_LOG("[ST] RESTORE_DONE\r\n");
    /* end timing */
    for (i = 0; i < NUM_CORE_STATES; i++)
    {
    ...
    if (log_this) STATE_LOG("[ST] BENCH_DONE crc=0x%04x\r\n", crc);
    return crc;
}
